        ...
        table: TABLE_NAME => {
            ...
            return {
                join: link => {
                    ...
                    return reference.table(TABLE_NAME)
                },
                getData: parameters => {
                    const mountQuery = tableMetadata => {
                        ...
                        if(this.joins && this.joins.length) {
                            this.joins.forEach((join, index)=> {
                                ...
                                if(join.type === 'left') { join.type = 'LEFT'; }
                                else if(join.type === 'right') { join.type = 'RIGHT'; }
                                ...
                                if(join.on.length) {
                                    ...
                                    join.on.forEach((link, linkIndex) => {
                                        ...
                                        query_joins.bindings += join.table.alias + '.' + link.column + '=' + TABLE_NAME + '.' + link.foreignColumn;
                                    })
                                }
                            ...
                            this.joins = []; // reset joins for further assistence
                        } // if has joins
                    ...
                    switch(reference.base.name) {
                        case 'mysql': {
                            return new Promise((resolve, reject) => {
                                ...
                                this.joins.forEach(join => { query_tables += ' OR TABLE_NAME=?'; tables.push(join.table.name) })
                                                ...
                                                if(blobIndexes.length) {
                                                    for(var i in results) {
                                                        for(var j in blobIndexes){
                                                            ...
                                                            if(results[i][resultsMD[blobIndex].COLUMN_NAME]) {
                        ...
                        case 'mssql': {
                            return new Promise((resolve, reject) => {
                                ...
                                this.joins.forEach((join, index) => { query_tables += (' OR TABLE_NAME=@TABLE_NAME' + index); tables.push(join.table.name) })
                                ...
                                db.connect(error => {
                                    ...
                                    if(parameters && (parameters.columns || parameters.where || parameters.group || parameters.order)) {
                                        ...
                                        this.joins.forEach((join, index) => { requestMetadata.input(('TABLE_NAME' + index), join.table.name) });
                ...
                setData: (object, parameters) => {
                    function parseObject(tableMetadata) {
                        ...
                        for(var i in tableMetadata) { // parse types and check nullables
