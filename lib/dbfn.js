    ...
    return {
            ...
            return {
                                    ...
                                    join.on.forEach((link, linkIndex) => {
                                        ...
                                        if(link.where && Object.keys(link.where).length) {
                                            Object.keys(link.where).forEach((key, keyIndex) => {
                                                const column = (/([0-9a-zA-Z_]*)( [0-9a-zA-Z_]*)?/).exec(key)[1]
                                                if(
                    ...
                    switch(reference.base.name) {
                        case 'mysql': {
                            return new Promise((resolve, reject) => {
                                let conn = reference.base.createConnection(reference.databaseOptions);
                                let tables = [TABLE_NAME];
                                let query_tables = 'TABLE_NAME=?';
                                let joins = parameters ? parameters.joins : null;
                                ...
                                if(parameters && (parameters.columns || parameters.where || parameters.group || parameters.order || parameters.datetimeFormat)) {
                                    conn.query({
                                        ...
                                        // DEFINE COLUMNS FOR PARSING
                                        let blobColumns = new Array();
                                        let datetimeColumns = new Array();
                                        for(let i in resultsMD) {
                                            let join = joins && joins.length ? joins.find(join => resultsMD[i].TABLE_NAME == join.table.name) : null;

                                            if((/blob/).test(resultsMD[i].COLUMN_TYPE)) {
                                                blobColumns.push(join ? `${join.table.alias}.${resultsMD[i].COLUMN_NAME}` : resultsMD[i].COLUMN_NAME);
                                            }
                                            if(parameters.datetimeFormat && (/datetime/).test(resultsMD[i].COLUMN_TYPE)) {
                                                datetimeColumns.push(join ? `${join.table.alias}.${resultsMD[i].COLUMN_NAME}` : resultsMD[i].COLUMN_NAME);
                                            }
                                        }
                                        ...
                                        conn.query({ sql: fn.sql, timeout: reference.databaseOptions.timeout || 5000 },
                                            (fn.where && fn.where.values ? fn.where.values : []), (error, results, fields) => {
                                                ...
                                                if(blobColumns.length) { // BLOB PARSING
                                                    for(let i in results) {
                                                        blobColumns.forEach(columnName => {
                                                ...
                                                if(datetimeColumns.length) { // DATETIME PARSING (formating)
                                                    for(let i in results) {
                                                        datetimeColumns.forEach(columnName => {
                                                            if(results[i][columnName]) {
                                                                // console.log('index:', i, columnName, results[i][columnName])
                                                                results[i][columnName] = moment(results[i][columnName], 'YYYY-MM-DD HH:mm').format(parameters.datetimeFormat);
                                                            }
                        ...
                        case 'mssql': {
                            return new Promise((resolve, reject) => {
                                const db = new reference.base.ConnectionPool(reference.databaseOptions);
                                const tables = [TABLE_NAME];
                                const query_tables = 'TABLE_NAME=@TABLE_NAME';
                                let joins = parameters ? parameters.joins : null;
                                ...
                                db.connect(error => {                                    
                                    ...
                                    if(parameters && (parameters.columns || parameters.where || parameters.group || parameters.order)) {
                                        const requestMetadata = db.request();
                                        const rsMetadata = { rows: [] }

                                        requestMetadata.stream = true;
                                        ...
                                        requestMetadata.on('done', (affected) => {
                                            ...
                                            request.on('done', (affected) => {
                                                ...
                                                for(let i in rsMetadata.rows) {
                                                    let join = joins && joins.length ? joins.find(join => rsMetadata.rows[i].TABLE_NAME == join.table.name) : null;

                                                    if(join) {
                                                        if((/blob/).test(rsMetadata.rows[i].DATA_TYPE)) {
                                                            blobColumns.push(join ? `${join.table.alias}.${rsMetadata.rows[i].COLUMN_NAME}` : rsMetadata.rows[i].COLUMN_NAME);
                                                        }
                                                        if(parameters.datetimeFormat && (/datetime/).test(rsMetadata.rows[i].DATA_TYPE)) {
                                                            datetimeColumns.push(join ? `${join.table.alias}.${rsMetadata.rows[i].COLUMN_NAME}` : rsMetadata.rows[i].COLUMN_NAME);
                                                        }
                                                    }        
                                                }
                ...
                setData: (object, parameters) => {
                    function parseObject(tableMetadata) {
                        ...
                        for(let k in object) { // remove unknown columns
                            if(_.findIndex(tableMetadata, c => c.COLUMN_NAME === k) === -1) delete object[k];
                        }

                        const requiredColumns = _.filter(tableMetadata, c => c.IS_NULLABLE === 'NO');    
                        
                        for(let i in requiredColumns) {
                            let column = requiredColumns[i];                            
                                                        
                            // there can't be integers since their value may be 0, hence would be considered 'undefined'            
                            if((_.findIndex(Object.keys(object), k => k === column.COLUMN_NAME) === -1 || // if column is found in sent 'object'
                    ...
                    switch(reference.base.name) {
                        case 'mysql': {
                            return new Promise((resolve, reject) => {
                                const conn = reference.base.createConnection(reference.databaseOptions);
                                conn.query({
                                    ...
                                    const checkInsertOrUpdate = new Promise((resolve, reject) => {
                                        const pkList = _.filter(resultsMD, r => r.COLUMN_KEY === 'PRI');
                                        let searchColumns = "";
                                        let searchValues = [];
                                    ...
                                    checkInsertOrUpdate.then(operation => {
                                        switch (operation) {
                                            ...
                                            case 'UPDATE': {
                                                ...
                                                const notPkList = _.filter(resultsMD, r => r.COLUMN_KEY !== 'PRI');
                            ...
                            return new Promise((resolve, reject) => {
                                ...
                                db.connect(error => {
                                    ...
                                    requestMetadata.on('done', affected => {
                                        ...
                                        const checkInsertOrUpdate = new Promise((resolve, reject) => {
                                            const pkList = _.filter(rsMetadata.rows, r => r.COLUMN_KEY === 'PRI');
                                            let searchColumns = "";
                                            let searchValues = [];
                                            if(pkList.length) { // has primary keys
                                                for(let i in pkList) {
                                                ...
                                                if(searchValues.length) { // if object has PK value set
                                                    const requestSearch = db.request();
                                                    const rsSearch = { rows: [] }
                                        ...
                                        checkInsertOrUpdate.then(operation => {
                                            switch (operation) {
                                                case 'INSERT': {
                                                    const requestExecute = db.request();
                                                    const rsExecute = { };
                                                    const objectKeys = Object.keys(object);
                                                    let requestInsertColumns = "";
                                                    let requestInsertValues = "";
                                                ...
                                                case 'UPDATE': {
                                                    ...
                                                    const rsExecute = { };
                                                    const pkList = _.filter(rsMetadata.rows, r => r.COLUMN_KEY === 'PRI');
                                                    const notPkList = _.filter(rsMetadata.rows, r => r.COLUMN_KEY !== 'PRI');
                                                    for(let i in notPkList) {
                                                        if(typeof object[notPkList[i].COLUMN_NAME] != 'undefined') {
        ...
        query: SQL => {
            ...
            return {
                execute: parameters => {
                    fn.values = [];
                    let sqlSets = SQL.match(/\{(\w+)\}/g) || [];
                    sqlSets = sqlSets.map(s => (/\{([\d\w_]*)\}/).exec(s)[1]);

                    sqlSets.forEach(key => fn.values.push(parameters[key]))
                    
                    switch(reference.base.name) {
                        case 'mysql': {
                            return new Promise((resolve, reject) => {
                                if(parameters && sqlSets && sqlSets.length > Object.keys(parameters).length) { reject("Missing parameters"); return;}
                                if(parameters) for(let key in parameters) SQL = SQL.replace(new RegExp(`{${key}}`, "g"), '?');
                        ...
                        case 'mssql': {
                            return new Promise((resolve, reject) => {
                                ...
                                if(parameters) for(let key in parameters) SQL = SQL.replace(new RegExp(`{${key}}`, "g"), `@${key}`);
                                ...
                                db.connect(error => {
                                    ...
                                    const request = db.request();
                                    const rs = { rows: [] }
                                    for(let p in parameters){ request.input(p, parameters[p]); }
                                    request.stream = true;
                                    request.query(SQL);
                                    // console.log(SQL);
                                    request.on('error', error => { reject('(SQL Error) ' + error.stack);  });
