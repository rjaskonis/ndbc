                                    ...
                                    join.on.forEach((link, linkIndex) => {
                                        ...
                                        if(link.where && Object.keys(link.where).length) {
                                            Object.keys(link.where).forEach((key, keyIndex) => {
                                                if(keyIndex > 0)  query_joins.bindings += ' AND ';

                                                if(
                                                    (/^[0-9a-zA-Z_]*$/).test(key) &&
                                                    _.findIndex(tableMetadata, md => md.COLUMN_NAME == key) !== -1  &&
                                                    ((/^[0-9a-zA-Z_]*$/).test(link.where[key]) || link.where[key] === null)
                                                ) {
                                                    let signal = link.where[key] === null ? ' IS ' : '=';
                                                    query_joins.bindings += join.table.alias + '.' + key + signal + (link.where[key] ? "'" + link.where[key] + "'" : 'NULL');
                                                }
                                            })
                                        }
                                    })
